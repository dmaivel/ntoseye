...
/// framing. Shared with the KD event-loop tests, which drive the same wire.
#[cfg(test)]
pub mod test_wire {
    ...
    /// A manipulate reply payload: header, union body, then trailing data.
    ...
    /// An ACK followed by a data packet: what comes back after a manipulate
