...
> [!WARNING]
> With VBS running, single-stepping over the GDB stub is unreliable, and that includes resuming from a breakpoint the vCPU is stopped on, which steps past it first. Windows runs nested under its own hypervisor, and a KVM single step of that vCPU can complete inside the hypervisor, with the trap flag still set in Windows. Windows then takes the single-step trap itself. In testing, a conditional breakpoint on `nt!KiSwapContext` resumed that way under a guest booted with `bcdedit /debug on` (KDNET) froze every CPU waiting for a kernel debugger, and the guest had to be reset. Use the `kd`/`kdnet` backends to step under VBS, or remove breakpoints before resuming over GDB.
