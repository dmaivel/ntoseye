...
On AMD64 QEMU/KVM, `gdb` additionally supports nonpatching hardware execution breakpoints (`ba e1`) in secure-kernel modules, real VTL1 register/stack inspection at those stops, stepping and run-to (`t`, `p`, `gu`, `g <address>`) through debug-register sites, and continue. Explicit `.vtl` memory selection alone supplies no register context. Software breakpoints and secure writes remain unsupported; see the linked usage section for the tested HVCI configuration and limits.
